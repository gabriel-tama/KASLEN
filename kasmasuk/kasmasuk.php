...
$result= mysqli_query($conn,"SELECT * from transaksi WHERE tipe=1");
$rows= [];
...
$result_ = mysqli_query($conn,"SELECT tanggal,tipe,jumlah,nama,deskripsi from transaksi JOIN users ON transaksi.user_id = users.user_id WHERE tipe=1  ORDER BY tanggal DESC LIMIT $awalData,$jumlahDataPerHalaman");
$rows= [];
...
if (isset($_POST["search"])){
	...
	$result= mysqli_query($conn,"SELECT tanggal,tipe,jumlah,nama,deskripsi from transaksi JOIN users ON transaksi.user_id = users.user_id WHERE tipe=1 AND nama LIKE '%$keyword%' ORDER BY tanggal DESC LIMIT $awalData,$jumlahDataPerHalaman");
